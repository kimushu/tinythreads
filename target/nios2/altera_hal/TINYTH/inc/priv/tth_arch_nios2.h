...
#error                                                                         \
    "Small C library is not thread-safe. Turn off 'hal.enable_small_c_library' to use thread-safe environment."
#endif
 ...
 */
static inline void tth_arch_cs_end(int status) { __builtin_wrctl(0, status); }
