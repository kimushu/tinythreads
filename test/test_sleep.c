...
static int64_t get_usec(void) {
  struct timeval tv;
  TEST_ASSERT_EQUAL_MESSAGE(0, gettimeofday(&tv, NULL),
...
static void test_usleep(void) {
  ...
  TEST_ASSERT_EQUAL_MESSAGE(EINVAL, errno, "errno should be set to EINVAL");

  start = get_usec();
  TEST_ASSERT_EQUAL_MESSAGE(0, usleep(200000),
                            "usleep() should succeed when duration is 200ms");
  int dur1 = get_usec() - start;

  start = get_usec();
  TEST_ASSERT_EQUAL_MESSAGE(0, usleep(400000),
...
static void test_sleep_seconds(void) {
  ...
  start = get_usec();
  TEST_ASSERT_EQUAL_MESSAGE(0, sleep(1),
