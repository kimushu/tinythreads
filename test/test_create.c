...
static void test_pthread_create_same_priority(void) {
  ...
  for (uint32_t timeout = 10000000; (timeout > 0) && (empty_worker_pass == 0);
       --timeout) {}
  TEST_ASSERT_TRUE_MESSAGE(
      empty_worker_pass != 0,
      "sub-thread should be started during main thread's busy loop");
  TEST_ASSERT_EQUAL_MESSAGE(0, pthread_join(tid, &retval),
                            "pthread_join() should succeed");
  TEST_ASSERT_EQUAL_MESSAGE(1, empty_worker_pass,
                            "sub-thread should be started in pthread_join()");
  TEST_ASSERT_EQUAL_MESSAGE((void *)(dummy ^ 0xdeadbeef), retval,
