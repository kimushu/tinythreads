...
static void *wait_worker(sem_t *sem)
{
  ...
  TEST_ASSERT_EQUAL_MESSAGE(
    0, sem_wait(sem),
    "sem_wait() on sub thread should succeed immediately");
...
static void test_sem_init_getvalue(void)
{
  ...
  TEST_ASSERT_EQUAL_MESSAGE(
    ENOSYS, errno,
    "errno should be set to ENOSYS");

  errno = 0;
  TEST_ASSERT_EQUAL_MESSAGE(
    -1, sem_init(&sem, 0, SEM_VALUE_MAX + 1),
  ...
  TEST_ASSERT_EQUAL_MESSAGE(
    0, sem_init(&sem, 0, 123),
    "sem_init() should succeed");

  int value = 0;
  TEST_ASSERT_EQUAL_MESSAGE(
    0, sem_getvalue(&sem, &value),
...
static void test_sem_post_overflow(void)
{
  ...
  memset(&sem, 0x55, sizeof(sem));
  TEST_ASSERT_EQUAL_MESSAGE(
    0, sem_init(&sem, 0, SEM_VALUE_MAX),
    "sem_init() should succeed");

  errno = 0;
  TEST_ASSERT_EQUAL_MESSAGE(
    -1, sem_post(&sem),
    "sem_destroy() should fail");
  TEST_ASSERT_EQUAL_MESSAGE(
    EOVERFLOW, errno,
