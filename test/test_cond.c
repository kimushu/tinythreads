...
static void *wait_worker(wait_worker_data *data)
{
  ...
  data->pass = 2;
  TEST_ASSERT_EQUAL_MESSAGE(
    0, pthread_cond_wait(data->cond, data->mutex),
...
static void test_PTHREAD_COND_INITIALIZER(void)
{
  ...
  TEST_ASSERT_EQUAL_MESSAGE(
    0, pthread_cond_init(&cond2, NULL),
    "pthread_cond_init() should succeed");
...
static void test_pthread_cond_wait_signal(void)
{
  ...
  TEST_ASSERT_EQUAL_MESSAGE(
    2, data1.pass,
    "Sub thread #1 should be waiting for condvar signal");
  TEST_ASSERT_EQUAL_MESSAGE(
    2, data2.pass,
    "Sub thread #2 should be waiting for condvar signal");
