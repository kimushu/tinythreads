...
extern int tth_arch_init_context(tth_thread *thread, void *stack_bottom,
...
static inline void tth_cs_move(tth_thread **from, tth_thread **to,
                               int waitstate) __attribute__((always_inline));
...
static inline int tth_is_interrupted(void) { return (tth_int_level != 0); }
